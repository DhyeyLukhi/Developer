...
def assist():  # assist: Ready for your command
    ...
    print("TIVIS - Your Desktop Assistant. Version 5.0")
    print("This is the MAX version of the TIVIS(Optimus)")
...
if __name__ == '__main__':  # The execution of the programme starts here
    ...
    while True:
        ...
        try:
            ...
            if 'tell me about' in command.lower() or 'according to google' in command.lower():
                command = command.replace("tell me about", "").replace("according to google", "")
                command = command.replace(" ", "+")
            ...
            if 'open' in command.lower() and ' website' in command.lower():
                ...
                if '.' in command.lower():
                    ...
                    l1 = ['net', 'org', 'in', 'gov.in', 'gov.us', 'com']
                    for i in range(0, len(l1)):
                        check = requests.get(f"https://www.{command}.{l1[i]}")
                        if 200 <= check.status_code < 300:
                            webbrowser.open(f"https://www.{command}.{l1[i]}")
                        else:
                            print("Opening...")
